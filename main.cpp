...
const char* const short_options="ge:d:k";
...
string string2bin(const string & plain){
    ...
    for(int i = 0; i<len;i++){
        ...
        for(int j = 64; j>=1; j/=2){
            result.append(to_string(temp/j));
            temp %= j;
        }
    }
    return result;
...
void bin2str(string &cipher){
    for(int i = 0; i<cipher.length();i+=7){
        string sub = cipher.substr(i, 7);
        const char* csub = sub.c_str();
        char* endptr;
        cout<<char(strtoul(csub, &endptr, 2));
    }
    cout<<endl;
}
...
void modify_string(string & plain, int n=64){
    // can be divided by n=64
    int len = plain.length();
    if(len%n!=0){
        for(int i=0; i<n-len%n;i++) plain.append("0");
    }
}

void getLR(const string & plain, int index, uint32_t & L, uint32_t & R){
    // change the value of L and R
    string subL = plain.substr(index, 32);
    string subR = plain.substr(index+32, 32);
    const char *cL = subL.c_str();
    const char *cR = subR.c_str();
    char* endptr;
    L = strtoul(cL, &endptr, 2);
    R = strtoul(cR, &endptr, 2);
}

string dec2bin_string(uint32_t n){
    string result;
    for(long int i = 2147483648; i>=1; i/=2){
        ...
        switch (next_option){
            ...
            case 'e':{
                ...
                if(next_option=='k'){
                    ...
                    keyFile.open(optarg);
                    keyFile>>keylen;
                    for(int i = 0; i<keylen;i++) keyFile>>key[i];
                    keyFile.close();
                }
                // initialize P and S
                for (int i=0 ; i<18 ; ++i)
                    P[i] ^= key[i % keylen];
                uint32_t L_0 = 0, R_0 = 0;
                ...
                string plain_bin = string2bin(plain);
                modify_string(plain_bin);
                string crypted_bin;
                int len = plain_bin.length();
                for(int i=0; i<len; i+=64){
                    uint32_t L=0, R=0;
                    // initialize L and R
                    getLR(plain_bin, i, L, R);
                    // encrypt
                    encrypt(L, R);
                    // append
            ...
            case 'd': {
                ...
                for(int i=0; i<length; i+=64){
                    ...
                    getLR(cipher, i, L, R);
                    decrypt(L, R);
                    message.append(dec2bin_string(L));
                    message.append(dec2bin_string(R));
                }
                bin2str(message);
            }
            case 'k':
